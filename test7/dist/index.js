...
const fetchWeather = async (InputCity) => {
    ...
    if (weather_img)
        weather_img.src = res.current.condition.icon;
    const day = document.querySelector("#day");
    if (day)
        day.innerHTML = res.current.condition.text;
    const time = document.querySelector("#time");
    if (time)
        time.innerHTML = res.location.localtime.split(' ')[1];
    ...
    if (inDegree)
        inDegree.innerHTML = res.current.dewpoint_c;
    const date = document.querySelector(".date");
    if (date)
        date.innerHTML = res.location.localtime.split(' ')[0];
};
fetchWeather("indore");
